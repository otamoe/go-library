...
func (stateMachine *StateMachine) Update(sEntries []dstatemachine.Entry) (rentries []dstatemachine.Entry, err error) {
	for sIndex, sEntry := range sEntries {
		...
		err = stateMachine.db.Update(func(txn *badger.Txn) (err error) {
			result.Data = make([]*libraftpb.Items, len(update.Entrys))
...
func (stateMachine *StateMachine) Lookup(e interface{}) (out interface{}, err error) {
	...
	// 遍历查询
	result.Data = make([]*libraftpb.Items, len(lookup.Entrys))
...
func (stateMachine *StateMachine) lookupEntry(txn *badger.Txn, entry *libraftpb.LookupEntry, resultField *libraftpb.ResultField) (pbItems *libraftpb.Items, err error) {
	// 查询
...
func (stateMachine *StateMachine) updateEntry(txn *badger.Txn, entry *libraftpb.UpdateEntry, eventBatch *StateMachineEventBatch, resultField *libraftpb.ResultField) (pbItems *libraftpb.Items, err error) {
	// 不创建 更新
...
func (stateMachine *StateMachine) query(txn *badger.Txn, pbQuery *libraftpb.Query, resultField *libraftpb.ResultField, cb func(item *badger.Item) (err error)) (err error) {
...
func (stateMachine *StateMachine) updateEntryOne(txn *badger.Txn, entry *libraftpb.UpdateEntry, eventBatch *StateMachineEventBatch, item *badger.Item, resultField *libraftpb.ResultField) (pbItem *libraftpb.Item, err error) {

	// 动作 匹配
	switch entry.Action {
	case libraftpb.UpdateAction_CREATE:
		// 创建  存在 跳过
		if item != nil {
			return
		}
	case libraftpb.UpdateAction_UPDATE:
		// 更新 不存在 返回
		if item == nil {
			return
		}
	case libraftpb.UpdateAction_DELETE:
		// 删除 不存在 返回
		if item != nil {
			return
		}
	...
	// 删除
	if entry.Expire == 1 || entry.Action == libraftpb.UpdateAction_DELETE {
		// 不存在 直接返回
		if item == nil {
			return
		}
...
func (stateMachine *StateMachine) toPBItem(item *badger.Item, resultField *libraftpb.ResultField) (pbItem *libraftpb.Item, err error) {
	if resultField.Key {
